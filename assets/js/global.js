...
$(function() {
	...
	$('.open-overlay').on('click', function(){
		var $this = $(this);
		var $overlay = $('.overlay[data-rel="'+$this.data('rel')+'"]');
		...
		if($overlay.hasClass('active')) {
			...
			if($this.hasClass('open-video')) setTimeout(function(){$overlay.find('iframe').attr('src', $this.data('src'));}, 500);
			if(_ismobile) setTimeout(function(){$('html').addClass('overflow-hidden');}, 500);
		}
	});

	//close overlay popup (keep this just in case other close buttons exist)
